...
function BaseLayout({ children }) {
  ...
  useEffect(() => {
    function handleClickOutside(event) {
      ...
      if (navMenuRef.current && !navMenuRef.current.contains(event.target) && event.target.tagName !== 'BUTTON' && event.target.className.includes('md:hidden')) {
        setIsNavOpen(false);
      }
    }
  ...
  }, [showProfileDropdown, isNavOpen]); // Include isNavOpen to re-run effect when nav state changes

  // Remove hamburger menu and mobile nav, show only profile or login button on mobile
  return (
      ...
      <nav className="bg-blue-900 p-4 shadow-lg">
        <div className="container mx-auto flex justify-between items-center">
          ...
          <div className="hidden md:flex items-center space-x-8">
            <button
              onClick={() => navigate('/')}
              className="text-white hover:text-blue-200 transition-colors duration-300 rounded-lg hover:bg-blue-800 text-base md:text-lg px-4 py-2 focus:outline-none"
            >
              Home
            </button>
            {user && user.is_staff && (
            ...
            {user && (
              <div className="relative ml-4" ref={profileBtnRef}>
                <button
                ...
                >
                  {user.profile_picture ? (
                    <img
                      ...
                      alt="Profile"
                      className="w-11 h-11 rounded-full border-2 border-yellow-400 object-cover shadow"
                    />
                  ) : (
                    <span className="flex items-center justify-center bg-yellow-400 rounded-full w-11 h-11">
                      <svg xmlns="http://www.w3.org/2000/svg" className="w-7 h-7 text-blue-900" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          {/* Mobile: Only show profile or login button */}
          <div className="flex md:hidden items-center">
            {user ? (
                ...
                >
                  {user.profile_picture ? (
                    <img
                      src={user.profile_picture}
                      alt="Profile"
                      className="w-9 h-9 rounded-full border-2 border-yellow-400 object-cover shadow"
                    />
                  ) : (
                    <span className="flex items-center justify-center bg-yellow-400 rounded-full w-9 h-9">
                      <svg xmlns="http://www.w3.org/2000/svg" className="w-6 h-6 text-blue-900" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <circle cx="12" cy="8" r="4" stroke="currentColor" strokeWidth="2" fill="none"/>
                        <path stroke="currentColor" strokeWidth="2" d="M4 20c0-4 4-7 8-7s8 3 8 7"/>
                      </svg>
                    </span>
              ...
              <button
                onClick={() => navigate('/login')}
                className="text-white bg-yellow-400 hover:bg-yellow-500 rounded-full px-5 py-2 font-semibold text-base shadow focus:outline-none"
              >
                Login
              </button>
            )}
