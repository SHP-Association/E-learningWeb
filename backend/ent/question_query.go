...
func (_q *QuestionQuery) QueryChoices() *AnswerChoiceQuery {
	query := (&AnswerChoiceClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *QuestionQuery) WithChoices(opts ...func(*AnswerChoiceQuery)) *QuestionQuery {
	query := (&AnswerChoiceClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(question.FieldQuestionText).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Question.Query().
//		Select(question.FieldQuestionText).
//		Scan(ctx, &v)
...
func (_q *QuestionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Question, error) {
	...
	if query := _q.withChoices; query != nil {
		if err := _q.loadChoices(ctx, query, nodes,
			func(n *Question) { n.Edges.Choices = []*AnswerChoice{} },
			func(n *Question, e *AnswerChoice) { n.Edges.Choices = append(n.Edges.Choices, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *QuestionQuery) loadChoices(ctx context.Context, query *AnswerChoiceQuery, nodes []*Question, init func(*Question), assign func(*Question, *AnswerChoice)) error {
	...
	query.withFKs = true
	query.Where(predicate.AnswerChoice(func(s *sql.Selector) {
