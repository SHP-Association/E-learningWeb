...
func (_q *EnrollmentQuery) QueryStudent() *UserQuery {
...
func (_q *EnrollmentQuery) QueryCertificate() *CertificateQuery {
...
func (_q *EnrollmentQuery) WithStudent(opts ...func(*UserQuery)) *EnrollmentQuery {
	query := (&UserClient{config: _q.config}).Query()
...
func (_q *EnrollmentQuery) WithCertificate(opts ...func(*CertificateQuery)) *EnrollmentQuery {
	query := (&CertificateClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(enrollment.FieldEnrolledAt).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Enrollment.Query().
//		Select(enrollment.FieldEnrolledAt).
//		Scan(ctx, &v)
...
func (_q *EnrollmentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Enrollment, error) {
	...
	if _q.withStudent != nil || _q.withCourse != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withStudent; query != nil {
		if err := _q.loadStudent(ctx, query, nodes, nil,
			func(n *Enrollment, e *User) { n.Edges.Student = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withCertificate; query != nil {
		if err := _q.loadCertificate(ctx, query, nodes, nil,
			func(n *Enrollment, e *Certificate) { n.Edges.Certificate = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *EnrollmentQuery) loadStudent(ctx context.Context, query *UserQuery, nodes []*Enrollment, init func(*Enrollment), assign func(*Enrollment, *User)) error {
...
func (_q *EnrollmentQuery) loadCertificate(ctx context.Context, query *CertificateQuery, nodes []*Enrollment, init func(*Enrollment), assign func(*Enrollment, *Certificate)) error {
	...
	query.Where(predicate.Certificate(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(enrollment.CertificateColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.enrollment_certificate
		if fk == nil {
			return fmt.Errorf(`foreign-key "enrollment_certificate" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "enrollment_certificate" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
