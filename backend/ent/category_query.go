...
func (_q *CategoryQuery) QueryCourses() *CourseQuery {
...
func (_q *CategoryQuery) QueryFaqs() *FAQQuery {
	query := (&FAQClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *CategoryQuery) WithCourses(opts ...func(*CourseQuery)) *CategoryQuery {
	query := (&CourseClient{config: _q.config}).Query()
...
func (_q *CategoryQuery) WithFaqs(opts ...func(*FAQQuery)) *CategoryQuery {
	query := (&FAQClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *CategoryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Category, error) {
	...
	if query := _q.withFaqs; query != nil {
		if err := _q.loadFaqs(ctx, query, nodes,
			func(n *Category) { n.Edges.Faqs = []*FAQ{} },
			func(n *Category, e *FAQ) { n.Edges.Faqs = append(n.Edges.Faqs, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *CategoryQuery) loadCourses(ctx context.Context, query *CourseQuery, nodes []*Category, init func(*Category), assign func(*Category, *Course)) error {
	...
	for _, n := range neighbors {
		fk := n.category_courses
		if fk == nil {
			return fmt.Errorf(`foreign-key "category_courses" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "category_courses" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *CategoryQuery) loadFaqs(ctx context.Context, query *FAQQuery, nodes []*Category, init func(*Category), assign func(*Category, *FAQ)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Category)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.FAQ(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(category.FaqsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.category_faqs
		if fk == nil {
			return fmt.Errorf(`foreign-key "category_faqs" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "category_faqs" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
