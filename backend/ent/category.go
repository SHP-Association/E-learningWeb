...
// was not loaded in eager-loading.
func (e CategoryEdges) FaqsOrErr() ([]*FAQ, error) {
	if e.loadedTypes[1] {
		return e.Faqs, nil
	}
...
func (*Category) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case category.FieldName, category.FieldSlug, category.FieldDescription, category.FieldImage:
...
func (_m *Category) QueryCourses() *CourseQuery {
	return NewCategoryClient(_m.config).QueryCourses(_m)
}

// QueryFaqs queries the "faqs" edge of the Category entity.
func (_m *Category) QueryFaqs() *FAQQuery {
	return NewCategoryClient(_m.config).QueryFaqs(_m)
}
